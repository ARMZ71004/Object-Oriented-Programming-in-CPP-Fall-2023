...
public:
    ...
    int& operator[](int index) {                // subscript member overload
        if(index < 0 || index > 4) { throw std::out_of_range ("index is out of range"); }   
        return values[index];                   // return the memory location of the variable at the specfied index                   
    }

    const int& operator[](int index) const {    // return by const value prevents the returned location from being edited after function ends
        if(index < 0 || index > 4) { throw std::out_of_range ("index is out of range"); }   
        return values[index];
    }
};
